...
class CommentController {
  ...
  async getComments (ctx) {
    ...
    if (payload) {
      for (let i = 0; i < list.length; i++) {
        const hands = await Hands.findOne({ uid: payload._id, cid: list[i]._id })
        if (hands) {
          list[i].isHands = '1'
        } else {
          list[i].isHands = '0'
  ...
  async updateComment (ctx) {
    ...
    if (result) {
      ...
      const updateResult = await Comment.updateOne({ _id: body.cid }, { $set: { content: body.content } })
  ...
  async setBestComment(ctx ) {
    ...
    const postResult = await Post.updateOne({ _id: body.tid }, { $set: { isEnd: '1' } })
    const commentResult = await Comment.updateOne({ _id: body.cid }, { $set: { isBest: '1' } })
    const userResult = await User.updateOne({ _id: comment.uid }, { $inc: { favs: parseInt(post.fav) } })
  ...
  async setHandsComment(ctx ) {
    ...
    if (hands) {
      const delResult = await Hands.deleteOne({ uid: payload._id, cid: body.cid })
      if (delResult.ok === 1) {
        ctx.body = {
          code: 10000,
          message: '取消点赞成功'
    ...
    await newHands.save()
    const result = await Comment.updateOne({ _id: body.cid }, { $inc: { hands: 1 } })
    if (result.ok === 1) {
      ctx.body = {
        code: 10000,
        message: '点赞成功'
