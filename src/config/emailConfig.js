...
async function sendEamil(emailInfo) {
  ...
  const template = `
                        <div style="width: 420px; margin: 20px auto; border: 1px solid #999;">
                          ...
                          <div style="padding: 20px 10px;">
                            <p style="margin: 0; font-size: 14px;">您好，<span style="color: #009688">${ emailInfo.userName }</span>攻城狮，重置链接有效时间为30分钟，请在${ emailInfo.expire }之前重置您的${ emailInfo.type === 'email'? '邮箱' : '密码' }</p>
                            <a style="display: inline-block; padding: 8px 6px; border: none; background-color: #009688; color: #fff; font-size: 12px; outline: none; margin: 10px 0; text-decoration: none;" href="${ jumpLink }">立即重置${ emailInfo.type === 'email'? '邮箱' : '密码' }</a>
                            <p style="margin: 0; font-size: 12px; background-color: #f2f2f2; padding: 8px 4px; color: #333;">如果不是本人操作，请勿进行重置！</p>
                          </div>
  ...
  let info = await transporter.sendMail({
    from: `重置${ emailInfo.type === 'email'? '邮箱' : '密码' } <1271250334@qq.com>`, // sender address
    to: emailInfo.email, // list of receivers
    subject: `《1024社区》确定重置${ emailInfo.type === 'email'? '邮箱' : '密码' }链接`, // Subject line
